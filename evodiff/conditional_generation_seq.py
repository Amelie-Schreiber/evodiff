...
def generate_scaffold_2(model, protein_sequence, scaffold_length, tokenizer, device='cuda', random_baseline=False):
    if random_baseline:
        # Assuming train_prob_dist is properly defined and moved to the correct device
        pass

    mask = tokenizer.mask_id

    # Tokenize the protein sequence
    motif_tokenized = tokenizer.tokenize(protein_sequence)
    
    # Ensure motif_tokenized is a sequence
    if isinstance(motif_tokenized, np.ndarray) and motif_tokenized.ndim == 0:
        motif_tokenized = [motif_tokenized.item()]

    # Create input motif + scaffold
    seq_len = scaffold_length+len(motif_tokenized)
    sample = torch.zeros((1, seq_len), dtype=torch.long) + mask  # start from all mask

    # Place motif at the beginning of the scaffold and move to the device
    len_motif = len(motif_tokenized)
    sample[:, :len_motif] = torch.tensor(motif_tokenized, dtype=torch.long).to(device)

    # Mask locations after the motif for scaffolding
    mask_locations = (sample[0] == mask).nonzero(as_tuple=True)[0]
    ...
    with torch.no_grad():
        for i in mask_locations:
            ...
            if random_baseline:
                p_sample = torch.multinomial(torch.tensor(train_prob_dist), num_samples=1).to(device)
            else:
                prediction = model(sample, timestep)
                p = prediction[:, i, :len(tokenizer.all_aas) - 6]  # only canonical
    ...
    untokenized = tokenizer.untokenize(sample[0].cpu())
...
def generate_autoreg_scaffold_from_seq_2(model, protein_sequence, additional_length, tokenizer, device='cuda', batch_size=1):
    mask = tokenizer.mask_id
    start = tokenizer.start_id
    stop = tokenizer.stop_id
    ...
    # Ensure motif_tokenized is an array and convert to list if needed
    if isinstance(motif_tokenized, np.ndarray):
        motif_tokenized = motif_tokenized.tolist()

    # Create input motif + scaffold
    seq_len = len(motif_tokenized) + additional_length
    max_seq_len = seq_len
    sample = torch.zeros((batch_size, seq_len), dtype=torch.long) + mask
    sample[:, :len(motif_tokenized)] = torch.tensor(motif_tokenized, dtype=torch.long)

    # Move sample to the specified device
    sample = sample.to(device)

    # Start from START token
    autoreg_sample = (torch.zeros((1, 1)) + start).long().to(device)
    reach_stop = False
    ...
    with torch.no_grad():
        i = 0
        while i < max_seq_len:
            if i >= len(motif_tokenized):  # Start scaffolding after the motif
                max_token = len(tokenizer.alphabet)
                if not reach_stop:
                    prediction = model(autoreg_sample, timestep)
                    p = prediction[:, -1, :max_token]
                    p = torch.nn.functional.softmax(p, dim=1)
                    p_sample = torch.multinomial(p, num_samples=1)
                    autoreg_sample = torch.cat((autoreg_sample, p_sample), dim=1)
                    if p_sample.item() == stop:
                        reach_stop = True
                    i += 1
                else:
                    break
                ...
                autoreg_sample = torch.cat((autoreg_sample, sample[:, i:i+1].to(device)), dim=1)
                i += 1

    untokenized = tokenizer.untokenize(autoreg_sample[0].cpu())[1:]  # Removing the start token
    generated_binder = untokenized[len(protein_sequence):]

    return untokenized, generated_binder

def generate_autoreg_scaffold_from_seq(model, motif_seq, motif_start_idxs, motif_end_idxs, scaffold_length, tokenizer,
                                       batch_size=1, device='cuda', single_res_domain=False):
    mask = tokenizer.mask_id  # placeholder to calculate indices here
    start = tokenizer.start_id
    stop = tokenizer.stop_id
    ...
    with torch.no_grad():
        ...
        while i < max_seq_len:
            ...
            if reach_stop == False:
                ...
                break
    print("new sequence", [tokenizer.untokenize(s) for s in sample[:,1:-1]])  # don't need start/stop tokens
    untokenized = [tokenizer.untokenize(s) for s in sample[:,1:-1]]

    return untokenized, new_start_idxs, new_end_idxs

def generate_binder(model, motif_seq, binder_length, tokenizer, batch_size=1, device='cuda'):
    mask = tokenizer.mask_id  # placeholder for indices calculation
    start = tokenizer.start_id
    stop = tokenizer.stop_id

    # Tokenize the provided motif sequence
    motif_tokenized = tokenizer.tokenize((motif_seq,))

    # Set up the sequence with motif and additional space for the binder
    seq_len = len(motif_seq) + binder_length
    max_seq_len = seq_len
    sample_ref = torch.zeros((batch_size, seq_len)) + mask  # start with all masked
    sample_ref[:, :len(motif_seq)] = torch.tensor(motif_tokenized)  # place motif at the beginning
    ...
    with torch.no_grad():
        ...
        while i < max_seq_len:
            if reach_stop == False:
                # Add residues until it predicts STOP token or hits max seq len
                prediction = model(sample, timestep)  # sample prediction given input
                p = prediction[:, -1, :max_token]  # predict next token
                p = torch.nn.functional.softmax(p, dim=1)  # softmax over categorical probs
                p_sample = torch.multinomial(p, num_samples=1)
                sample = torch.cat((sample, p_sample), dim=1)
    ...
    # Ensure we are working with a single sequence (first row of the tensor)
    if sample.ndim > 1:
        sample = sample[0]

    # Extract generated scaffold and binder
    generated_scaffold = tokenizer.untokenize(sample[1:-1])  # exclude start/stop tokens
    generated_binder = generated_scaffold[len(motif_seq):]  # extract only the generated binder part

    return generated_binder
...
def inpaint_simple(model, sequence, start_idx, end_idx, tokenizer=Tokenizer(), device='cuda'):
    ...
    tokenized_sequence = torch.tensor(tokenizer.tokenizeMSA(sequence)).to(device)
